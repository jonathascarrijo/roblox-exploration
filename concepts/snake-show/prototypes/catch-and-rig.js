...
(() => {
  ...
  function render() {
    ...
    $('rigStatus').textContent = s.armed ? `Rig armed · ${burst > 0 ? '2× motor burst: ' + burst.toFixed(1) + ' s' : s.burstUntil === s.time && s.privateLog.at(-1)?.text.includes('rescue area') ? 'armed from the rescue area; no motor changed' : 'burst ended; diverter still armed'}` : s.spent ? `Attempt spent. ${s.heists ? 'Private outcome: one heist succeeded.' : 'No capsule is armed.'}` : s.reserved !== null ? 'Ordinary Pull is active throughout this hold.' : 'Rig also sends ordinary Pull during the hold.';
