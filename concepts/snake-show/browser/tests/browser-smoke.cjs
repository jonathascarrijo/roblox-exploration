...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.evaluate(() => snakeShowTest.game.players.filter(p => p.role === 'Snake').length), 2);
    assert.match(await page.locator('#privateRole').innerText(), /Snake/);
    ...
    assert.doesNotMatch(await page.locator('#receipts').innerText(), /Rig|diverter|Snake channel|doubled/);
